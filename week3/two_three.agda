...
ex5 (case1 fA) ⟪ a , b ⟫ = fA a
ex5 (case2 fB) ⟪ a , b ⟫ = fB b
-- record { proj1 = a ;  proj2 = a}
data ⊥ : Set where
...
module tmp ( Cat Dog Goat Rabbit : Set ) (p :  PetResearch  Cat Dog Goat Rabbit ) where
    ...
    sub1 : Goat → ¬ Cat
    sub1 g c = fact1 p (case1 c) g

    sub2 : Goat → Set
    sub2 g = ⊥

    extractRabbit : (dr : Dog ∨ Rabbit) → ¬ Dog → Rabbit
    extractRabbit (case1 dog) ¬dog = ⊥-elim (¬dog dog)
    extractRabbit (case2 rabbit) ¬dog = rabbit
    

    problem2 : Goat → Rabbit
    problem2 g = extractRabbit (fact2 p (sub1 g)) (problem1 g)
      --  lemma1 : ¬ (Cat ∨ Goat)
      --  lemma1 (case1 cat) =  sub1 g cat
      --  lemma1 (case2 goat) = extractRabbit 
